...
class MainController(rpc: NodeRPCConnection) {
    ...
    fun createIOU(request: HttpServletRequest): ResponseEntity<String> {
        ...
        val productName = request.getParameter("productName")
        val productColour = request.getParameter("productColour")
        ...
        return try {
            proxy.startTrackedFlow(::Initiator, otherParty,productName,productColour,status).returnValue.getOrThrow()

            val signedTxWithStatusAsRecieved = proxy.startTrackedFlow(::Initiator, otherParty,productName,productColour,status).returnValue.getOrThrow()

            ResponseEntity.status(HttpStatus.CREATED).body("Transaction id ${signedTxWithStatusAsRecieved.id} committed to ledger.\n")
